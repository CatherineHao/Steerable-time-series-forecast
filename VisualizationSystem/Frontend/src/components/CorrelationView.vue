        ...
        <div ref="CorrelationView" style="height: 100%; width: 100%;">
            ...
            <svg id="CorrelationView" height="100%" width="100%">
                ...
                <g :transform="translate(55, 30, 0)">
                    ...
                    <rect v-for="(o, i) in 36" :key="'st' + i" :x="(i % 9) * (elWidth - 60) / 9" :y="Math.floor(i / 9) * (elHeight - 30) / 4" :width="(elWidth - 60) / 9" :height="(elHeight - 30) / 4" fill="none" stroke="black"></rect>
                </g>
            </svg>
        </div>
    </div>
...
import { interpolateRdBu, interpolateYlOrRd, scaleLinear } from 'd3';
export default {
        ...
        calcData (data) {
            ...
            for (let i = 0; i < 9; ++i) {
                for (const j in data[i].predic_info) {
                    ...
                    tmp['x1'] = i * (this.elWidth - 60) / 9;
                    tmp['w1'] = 6 * (this.elWidth - 60) / (9 * 7);
                    tmp['x2'] = i * (this.elWidth - 60) / 9 + tmp['w1'];
                    tmp['w2'] = (this.elWidth - 60) / (9 * 7);
